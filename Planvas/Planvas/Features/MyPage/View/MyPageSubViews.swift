...
struct DetailPageView: View {
    ...
    var body: some View {
        VStack(spacing: 40) {
           menuSection("목표 및 활동 관리") {
               menuButton(route: .mainPage,title: "현재 목표 수정하기", desc: "비율 및 기간 변경")
               menuButton(route: .reportPage, title: "지난 시즌 리포트", desc: "히스토리 모아보기")
           }
           menuSection("연동 및 알림") {
                menuButton(route: .calenderPage, title: "캘린더 연동 설정", desc: "구글 캘린더 관리")
                menuButton(route: .alarmPage, title: "알림 및 리마인더", desc: "D-day 및 완료 알림")
            }
    ...
    @ViewBuilder
    private func menuButton(route: MyPageRoute, title: String, desc: String) -> some View {
